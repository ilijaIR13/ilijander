...
export default async function ProfilePage() {
  ...
  if (solvedRows?.length) {
    ...
    solved = solvedRows.map(r => ({ ...map.get(r.task_id), done_at: r.done_at })).filter(Boolean);
  }
  ...
  return (
    <Container>
      ...
      <div className="grid gap-3">
        {solved.length ? solved.map(t => (
          <div key={t.id} className="card p-4 flex items-center justify-between">
            <Link href={`/tasks/${t.slug}`} className="hover:underline">{t.title}</Link>
            <span className="text-xs text-slate-500">{t.done_at ? new Date(t.done_at).toLocaleString() : ""}</span>
          </div>
